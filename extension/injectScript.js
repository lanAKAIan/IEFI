...
(function() {
	...
	*/
    ...
    var dashboardConstFuncName = "si"; /*DASHBOARD_UPDATE_LINE in input 120ish*/
    function setUpKnownHooks()
    {
        hooks.getMap            = function(){return (vf.f().n); } /*DASHBOARD_UPDATE_LINE in doGeocode, input 83ish, then in input 118ish*/
        ...
        hooks.valueFromCookie   = function(name){ return(td(name)) }; /*DASHBOARD_UPDATE_LINE in input 94ish*/
    ...
    function overrideDashboard()
    {
        ...
        if(transientData.compatibility == "compatible")
        {
            if(userData.userSettings.comm_default_chat_tab != "faction")
            {
                ...
                replaceInFunction(dashboardConstFuncName,commVar[0],commVar[0]+newLine); /*this is the same text as hooks.dashboardConstructor*/
            }
...
*/
            ...
            if(userData.userSettings.auto_load_fresh != "world" || userData.userSettings.auto_load_page != "last")
            {
                ...
                var matches = signatures.dashboardConst.exec(window[dashboardConstFuncName].toString()); /*this is the same as hooks.dashboardConstructor*/
